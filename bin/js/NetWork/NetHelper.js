...
(function (smallGame) {
    ...
    (function (NetHelper) {
        function SVR_SET_CMD(command, data) {
            var packet = new smallGame.NetPacket();
            packet._cmd = command;
            packet._errorCode = 0;
            packet._data = data;
            return packet;
        ...
        function SENDMSG(packet) {
            return smallGame.GameManager.getInstace().getServerDispatcher().sendMsg(packet);
        }
        NetHelper.SENDMSG = SENDMSG;
        function C2S_SEND(cmd, data) {
            var ser = smallGame.GameManager.getInstace().getServerDispatcher();
    ...
    })(NetHelper = smallGame.NetHelper || (smallGame.NetHelper = {}));
})(smallGame || (smallGame = {}));
